...
import { fetchAdminWithdrawals, type IWithdrawal } from './withdrawall.api';

const ITEMSIZE = 50;
const TableOptions: ITxCoolTableOption = { headStyles, bodyStyles };

export default function WithdrawalList() {
  const [data, _data] = useState<IWithdrawal[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!partnerId) return;
      const res = await fetchAdminWithdrawals({ offset: (pageIdx - 1) * ITEMSIZE, limit: ITEMSIZE, partnerId, keyword });
  ...
  return (
    <div className="flex flex-1 flex-col">
      <div className="flex items-end justify-between gap-3 mb-4">
        <TxDropdownPatners onChangeText={_partnerId} pageRole={SYS_PAGE_ROLE.ADMIN} />
