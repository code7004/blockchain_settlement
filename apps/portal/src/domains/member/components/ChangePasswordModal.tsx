...
import { apiPatchChangePwd, apiVerifyPwd } from '../member.api';
...
export const ChangePasswordModal = ({ auth, ...props }: ITxModalUserForm) => {
  ...
  const hdCheckVerifyForm = async () => {
    ...
    try {
      await apiVerifyPwd({ id: auth.id, password: verifyForm.password1 });
      _step(1);
      _eMessage(undefined);
    } catch (err) {
  ...
  const hdModify = networkWrapper(async () => {
    ...
    try {
      await apiPatchChangePwd({ id: auth.id, oldPassword: verifyForm.password2, newPassword: changeForm.password2 });
      _step(1);
      _eMessage(undefined);
      alert('비밀변호가 변경되었습니다.');
      hdExit();
