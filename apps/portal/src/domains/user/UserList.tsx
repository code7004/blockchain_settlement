...
export default function UserList({ tableOptions, pageRole }: { pageRole: SYS_PAGE_ROLE; tableOptions: ITxCoolTableOption }) {
  const [filter, _filter] = useStateForObject<{ pageIdx: number; partnerId: string; isActive?: boolean }>({ pageIdx: 1, partnerId: '', isActive: undefined });
  ...
  const { data, refetch, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await getUsers({ offset: (filter.pageIdx - 1) * ITEMSIZE, limit: ITEMSIZE, partnerId: filter.partnerId, isActive: filter.isActive });
      return { data: (res.data?.map((e, idx) => ({ IDX: idx + 1, ...e })) as UserDto[]) ?? [], total: res.total };
    },
    enabled: !!filter.partnerId && filter.partnerId != '', // block condition
    staleTime: 1000 * 10,
    refetchInterval: 10000,
  });

  const validateForm = () => {
    if (filter.partnerId == '') return { partnerId: 'partnerId을 입력하세요' };
  ...
  async function hdCreateItem() {
    try {
      ...
      await postAdminUser({ partnerId: filter.partnerId, externalUserId: form.externalUserId });
      await refetch();
      _filter({ pageIdx: 1 });
      _eMessage(undefined);
    } catch (err) {
